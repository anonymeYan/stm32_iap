...
int32_t Ymodem_Receive (uint8_t *buf)
{
    ...
    int32_t i, j, packet_length, session_done, file_done, packets_received, errors,errorNew=0, session_begin, size = 0;
                    ...
                    else
                    {
                        if (packets_received == 0)//文件信息(首包)
                        {
                            ...
                            if (packet_data[PACKET_HEADER] != 0)//文件名字
                            {
                                ...
                                if (size > ((FLASH_SIZE-0x10000)/2 - 1))  //程序最大96K
                                {
                                    /* End session */
                                    Send_Byte(CA);
                                    Send_Byte(CA);
