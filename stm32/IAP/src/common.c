...
uint8_t EraseSomePages(__IO uint32_t size, uint8_t outPutCont)
{
    ...
    for (EraseCounter = 0; (EraseCounter < NbrOfPage) && (FLASHStatus == FLASH_COMPLETE); EraseCounter++)
    {
        ...
        if(outPutCont == 1)
        {
            ...
            Int2Str(erase_cont, (EraseCounter + 1)*2);//一页大小是2K
            SerialPutString(erase_cont);
            SerialPutString("K@\r\n");
            if(EraseCounter == NbrOfPage-1)
            {
                SerialPutString(" @Erase complete Success");
            }
        }
    }
    FLASH_Lock();
...
uint8_t EraseBuf(void)
{
    ...
    for (EraseCounter = 0; (EraseCounter < NbrOfPage) && (FLASHStatus == FLASH_COMPLETE); EraseCounter++)
    {
        FLASHStatus = FLASH_ErasePage(BufAddress + (PAGE_SIZE * EraseCounter));
        if(1)
        {
            SerialPutString(" @erase");
            Int2Str(erase_cont, (EraseCounter + 1)*2);//一页大小是2K
            ...
            if(EraseCounter == NbrOfPage-1)
            {
                SerialPutString(" @Erase Buf complete Success");
            }
        }
    }
    FLASH_Lock();
...
uint8_t EraseAPP(void)
{
    ...
    for (EraseCounter = 0; (EraseCounter < NbrOfPage) && (FLASHStatus == FLASH_COMPLETE); EraseCounter++)
    {
        ...
        if(1)
        {
            ...
            if(EraseCounter == NbrOfPage-1)
            {
                SerialPutString(" @Erase APP complete Success");
            }
        }
    }
    FLASH_Lock();
